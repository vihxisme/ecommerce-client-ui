...
const pageTitle = ref("Áo khoác");

const filterTitle = ref("Bộ lọc");
const showFilter = ref(false);

const jacketProduct = ref({
...
const displayedProducts = ref([...jacketProduct.value.product]);
const pageNumber = ref(jacketProduct.value.pageNumber);
const totalPage = ref(jacketProduct.value.totalPage);
const loading = ref(false);

// Kiểm tra còn dữ liệu để tải không
const noMoreData = computed(() => displayedProducts.value.length >= jacketProduct.value.pageSize);
...
const fetchMoreProducts = async () => {
  ...
  setTimeout(() => {
    ...
    if (displayedProducts.value.length + moreProducts.length >= jacketProduct.value.totalElement) {
      displayedProducts.value.push(...moreProducts.slice(0, jacketProduct.value.totalElement - displayedProducts.value.length));
    } else {
      displayedProducts.value.push(...moreProducts);
    }

    // Kiểm tra lại điều kiện dừng tải
    if (displayedProducts.value.length >= jacketProduct.value.totalElement) {
...
const changePage = async (newPage) => {
  ...
  displayedProducts.value = [...jacketProduct.value.product]; // Reset danh sách
