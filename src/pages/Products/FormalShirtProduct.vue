...
const pageTitle = ref("Áo sơ mi");

const filterTitle = ref("Bộ lọc");
const showFilter = ref(false);

const formalShirtProduct = ref({
...
const displayedProducts = ref([...formalShirtProduct.value.product]);
const pageNumber = ref(formalShirtProduct.value.pageNumber);
const totalPage = ref(formalShirtProduct.value.totalPage);
const loading = ref(false);

// Kiểm tra còn dữ liệu để tải không
const noMoreData = computed(() => displayedProducts.value.length >= formalShirtProduct.value.pageSize);
...
const fetchMoreProducts = async () => {
  ...
  setTimeout(() => {
    ...
    if (displayedProducts.value.length + moreProducts.length >= formalShirtProduct.value.totalElement) {
      displayedProducts.value.push(...moreProducts.slice(0, formalShirtProduct.value.totalElement - displayedProducts.value.length));
    } else {
      displayedProducts.value.push(...moreProducts);
    }

    // Kiểm tra lại điều kiện dừng tải
    if (displayedProducts.value.length >= formalShirtProduct.value.totalElement) {
...
const changePage = async (newPage) => {
  ...
  displayedProducts.value = [...formalShirtProduct.value.product]; // Reset danh sách
