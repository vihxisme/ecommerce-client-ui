...
const pageTitle = ref("Quần Jean");

const filterTitle = ref("Bộ lọc");
const showFilter = ref(false);

const jeansProduct = ref({
...
const displayedProducts = ref([...jeansProduct.value.product]);
const pageNumber = ref(jeansProduct.value.pageNumber);
const totalPage = ref(jeansProduct.value.totalPage);
const loading = ref(false);

// Kiểm tra còn dữ liệu để tải không
const noMoreData = computed(() => displayedProducts.value.length >= jeansProduct.value.pageSize);
...
const fetchMoreProducts = async () => {
  ...
  setTimeout(() => {
    ...
    if (displayedProducts.value.length + moreProducts.length >= jeansProduct.value.totalElement) {
      displayedProducts.value.push(...moreProducts.slice(0, jeansProduct.value.totalElement - displayedProducts.value.length));
    } else {
      displayedProducts.value.push(...moreProducts);
    }

    // Kiểm tra lại điều kiện dừng tải
    if (displayedProducts.value.length >= jeansProduct.value.totalElement) {
...
const changePage = async (newPage) => {
  ...
  displayedProducts.value = [...jeansProduct.value.product]; // Reset danh sách
