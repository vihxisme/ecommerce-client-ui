...
const pageTitle = ref("Quần Short");

const filterTitle = ref("Bộ lọc");
const showFilter = ref(false);

const shortsProduct = ref({
...
const displayedProducts = ref([...shortsProduct.value.product]);
const pageNumber = ref(shortsProduct.value.pageNumber);
const totalPage = ref(shortsProduct.value.totalPage);
const loading = ref(false);

// Kiểm tra còn dữ liệu để tải không
const noMoreData = computed(() => displayedProducts.value.length >= shortsProduct.value.pageSize);
...
const fetchMoreProducts = async () => {
  ...
  setTimeout(() => {
    ...
    if (displayedProducts.value.length + moreProducts.length >= shortsProduct.value.totalElement) {
      displayedProducts.value.push(...moreProducts.slice(0, shortsProduct.value.totalElement - displayedProducts.value.length));
    } else {
      displayedProducts.value.push(...moreProducts);
    }

    // Kiểm tra lại điều kiện dừng tải
    if (displayedProducts.value.length >= shortsProduct.value.totalElement) {
...
const changePage = async (newPage) => {
  ...
  displayedProducts.value = [...shortsProduct.value.product]; // Reset danh sách
