...
const pageTitle = ref("Áo Sweater");

const filterTitle = ref("Bộ lọc");
const showFilter = ref(false);

const sweaterProduct = ref({
...
const displayedProducts = ref([...sweaterProduct.value.product]);
const pageNumber = ref(sweaterProduct.value.pageNumber);
const totalPage = ref(sweaterProduct.value.totalPage);
const loading = ref(false);

// Kiểm tra còn dữ liệu để tải không
const noMoreData = computed(() => displayedProducts.value.length >= sweaterProduct.value.pageSize);
...
const fetchMoreProducts = async () => {
  ...
  setTimeout(() => {
    ...
    if (displayedProducts.value.length + moreProducts.length >= sweaterProduct.value.totalElement) {
      displayedProducts.value.push(...moreProducts.slice(0, sweaterProduct.value.totalElement - displayedProducts.value.length));
    } else {
      displayedProducts.value.push(...moreProducts);
    }

    // Kiểm tra lại điều kiện dừng tải
    if (displayedProducts.value.length >= sweaterProduct.value.totalElement) {
...
const changePage = async (newPage) => {
  ...
  displayedProducts.value = [...sweaterProduct.value.product]; // Reset danh sách
