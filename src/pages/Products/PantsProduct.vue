...
const pageTitle = ref("Quần");

const filterTitle = ref("Bộ lọc");
const showFilter = ref(false);

const pantsProduct = ref({
...
const displayedProducts = ref([...pantsProduct.value.product]);
const pageNumber = ref(pantsProduct.value.pageNumber);
const totalPage = ref(pantsProduct.value.totalPage);
const loading = ref(false);

// Kiểm tra còn dữ liệu để tải không
const noMoreData = computed(() => displayedProducts.value.length >= pantsProduct.value.pageSize);
...
const fetchMoreProducts = async () => {
  ...
  setTimeout(() => {
    ...
    if (displayedProducts.value.length + moreProducts.length >= pantsProduct.value.totalElement) {
      displayedProducts.value.push(...moreProducts.slice(0, pantsProduct.value.totalElement - displayedProducts.value.length));
    } else {
      displayedProducts.value.push(...moreProducts);
    }

    // Kiểm tra lại điều kiện dừng tải
    if (displayedProducts.value.length >= pantsProduct.value.totalElement) {
...
const changePage = async (newPage) => {
  ...
  displayedProducts.value = [...pantsProduct.value.product]; // Reset danh sách
