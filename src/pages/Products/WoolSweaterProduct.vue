...
const pageTitle = ref("Áo len");

const filterTitle = ref("Bộ lọc");
const showFilter = ref(false);

const woolSweaterProduct = ref({
...
const displayedProducts = ref([...woolSweaterProduct.value.product]);
const pageNumber = ref(woolSweaterProduct.value.pageNumber);
const totalPage = ref(woolSweaterProduct.value.totalPage);
const loading = ref(false);

// Kiểm tra còn dữ liệu để tải không
const noMoreData = computed(() => displayedProducts.value.length >= woolSweaterProduct.value.pageSize);
...
const fetchMoreProducts = async () => {
  ...
  setTimeout(() => {
    ...
    if (displayedProducts.value.length + moreProducts.length >= woolSweaterProduct.value.totalElement) {
      displayedProducts.value.push(...moreProducts.slice(0, woolSweaterProduct.value.totalElement - displayedProducts.value.length));
    } else {
      displayedProducts.value.push(...moreProducts);
    }

    // Kiểm tra lại điều kiện dừng tải
    if (displayedProducts.value.length >= woolSweaterProduct.value.totalElement) {
...
const changePage = async (newPage) => {
  ...
  displayedProducts.value = [...woolSweaterProduct.value.product]; // Reset danh sách
