...
const pageTitle = ref("Áo");

const filterTitle = ref("Bộ lọc");
const showFilter = ref(false);

const shirtProduct = ref({
...
const displayedProducts = ref([...shirtProduct.value.product]);
const pageNumber = ref(shirtProduct.value.pageNumber);
const totalPage = ref(shirtProduct.value.totalPage);
const loading = ref(false);

// Kiểm tra còn dữ liệu để tải không
const noMoreData = computed(() => displayedProducts.value.length >= shirtProduct.value.pageSize);
...
const fetchMoreProducts = async () => {
  ...
  setTimeout(() => {
    ...
    if (displayedProducts.value.length + moreProducts.length >= shirtProduct.value.totalElement) {
      displayedProducts.value.push(...moreProducts.slice(0, shirtProduct.value.totalElement - displayedProducts.value.length));
    } else {
      displayedProducts.value.push(...moreProducts);
    }

    // Kiểm tra lại điều kiện dừng tải
    if (displayedProducts.value.length >= shirtProduct.value.totalElement) {
...
const changePage = async (newPage) => {
  ...
  displayedProducts.value = [...shirtProduct.value.product]; // Reset danh sách
