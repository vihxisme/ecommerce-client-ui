          ...
          <v-col cols="12" lg="9" class="p-0">
            <v-container fluid>
              ...
              <v-row>
                ...
                <v-col cols="12" sm="4" md="3" lg="3" xl="2" v-for="product in prodByCateApparelType" :key="product.id"
...
import { getProdByCateApparelType } from "@/services/apis/productService";
...
const prodByCateApparelType = ref([]);
const pageNumber = ref(1);
const pageSize = ref(5);
const totalPages = ref();
const totalElements = ref();
const loading = ref(false);
const apparelType = computed(() => Number(route.params.apparelType));

const categoryStore = useCategoryStore();

const fetchProdByCateApparelType = async () => {
  loading.value = true;
  try {
    const request = {
    ...
    const res = await getProdByCateApparelType(apparelType.value, request);
    prodByCateApparelType.value = res.data.data.content;
...
const updatePageTitle = () => {
  if (route.params.apparelType === "1") {
    pageTitle.value = "Áo";
  } else {
    pageTitle.value = "Quần";
  }
};

onMounted(() => {
  updatePageTitle();
  fetchProdByCateApparelType();
})
...
watch(() => route.params.apparelType, () => {
  updatePageTitle();
  fetchProdByCateApparelType();
});
...
// Hàm sắp xếp
const sortProducts = () => {
  prodByCateApparelType.value.sort((a, b) => {
    switch (sortBy.value) {
      case "price_asc":
