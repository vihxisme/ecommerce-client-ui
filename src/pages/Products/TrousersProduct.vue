...
const pageTitle = ref("Quần Âu");

const filterTitle = ref("Bộ lọc");
const showFilter = ref(false);

const trousersProduct = ref({
...
const displayedProducts = ref([...trousersProduct.value.product]);
const pageNumber = ref(trousersProduct.value.pageNumber);
const totalPage = ref(trousersProduct.value.totalPage);
const loading = ref(false);

// Kiểm tra còn dữ liệu để tải không
const noMoreData = computed(() => displayedProducts.value.length >= trousersProduct.value.pageSize);
...
const fetchMoreProducts = async () => {
  ...
  setTimeout(() => {
    ...
    if (displayedProducts.value.length + moreProducts.length >= trousersProduct.value.totalElement) {
      displayedProducts.value.push(...moreProducts.slice(0, trousersProduct.value.totalElement - displayedProducts.value.length));
    } else {
      displayedProducts.value.push(...moreProducts);
    }

    // Kiểm tra lại điều kiện dừng tải
    if (displayedProducts.value.length >= trousersProduct.value.totalElement) {
...
const changePage = async (newPage) => {
  ...
  displayedProducts.value = [...trousersProduct.value.product]; // Reset danh sách
